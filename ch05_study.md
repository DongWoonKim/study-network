...
##### 호스트A에서 호스트D로 이더넷 프레임 전송 SW1d의 동작 
...
>
> Unknown 유니캐스트 프레임은 수신한 포트를 제외한 모든 포트에 전송한다. 이 동적을 <strong>플러딩</strong>이라고 한다. 레이어2 스위치의 이더넷 프레임 전송은 '모르면 일단 전송해 둔다.'라는 식으로 조금 무책임하게 동작한다. 이런 점은 라우터의 동작과 크게 다르다. 라우터의 경우는 목적지를 모르면 데이터를 파기한다.
>
